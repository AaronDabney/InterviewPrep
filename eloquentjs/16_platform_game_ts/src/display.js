...
function scrollPlayerIntoView(gameState, displayState) {
    ...
    var player = (0, gameState_1.getPlayerFromGameState)(gameState);
    var playerDisplayOffset = vector2.mult(player.size, 0.5);
    var center = vector2.mult(vector2.add(player.position, playerDisplayOffset), displayState.scale);
...
function drawActors(actors, scale) {
    var output = buildNode.apply(void 0, __spreadArray(["div", [['class', 'actor-layer']]], actors.map(function (actor) {
